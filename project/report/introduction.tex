...
In particular, they provide a precise framework to model constant-time properties(~\secref{prelimnaries}), a sound and complete reduction of constant-timedness of input programs to assertion safety of a self-product(~\secref{body}) and design, evaluate an automated tool, \texttt{ct-verif}, that verifies the constant-timedness of LLVM implementations of cryptographic algorithms from widely used libraries within seconds(~\secref{eval}). 
